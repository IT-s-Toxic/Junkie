...
def init(client):
    ...
    async def summarize_handler(event):
        ...
        await event.reply("Ща, жди. Прочитаю всё и жахну")
        if not match:
            return
...
async def _do_summarize(msgs):
    ...
    prompt = (
        ...
        "представляющий собой переписку в чате между несколькими участниками. "
        "Сформируйте краткую, понятную суммаризацию, в которой будет указано, кто из участников и о чём говорил. "
        "Выделяйте ключевые идеи, факты и основные детали обсуждений, при этом не добавляйте домыслов и не искажайте содержание. "
        "Обратите внимание на имена участников и темы, которые они обсуждали. "
    	"В обязательном порядке указыайте какой участник чата и о чём говорил в формате: имя пользователя - о чём говорил"
        "Все ответы должны быть исключительно на русском языке. "
        "Использование других языков строжайше запрещено!\n\n"
        "Вот текст для суммирования:\n\n"
        f"{combined_text}\n\n"
        "Требуется краткое резюме (не более 300 слов), в котором будут выделены главные аспекты и суть обсуждений, "
        "с указанием, кто из участников и о чём говорил. "
        "Резюме должно быть написано на естественном русском языке и только на нём. "
        "Если какие-то детали не нужны для понимания сути, опустите их. "
